...
{$ELSE}
{$ENDIF}
  ExtCtrls, Printers, DBGridEh;//, PrnDbgeh;

type

  TViewMode = (vm500, vm200, vm150, vm100, vm75, vm50, vm25, vm10, vmPageWidth, vmFullPage);
  TPrinterChangePageHeader = procedure(Value: string) of object;
  TPrinterChangeHeadFont = procedure(Font: TFont) of object;
  TPrinterChangePostion = procedure(left: Integer=0; right: Integer=0; top: Integer=0; bottom: Integer=0) of object;
...
{ TPrinterPreview }
  ...
  private
    ...
    FOnPrinterSetupChanged: TNotifyEvent;
    FOnPrinterSetupDialog: TNotifyEvent;
    FOnPrinterChangePageHeader: TPrinterChangePageHeader;
    FOnPrinterChangeHeadFont: TPrinterChangeHeadFont;
    FOnPrinterChangePostion: TPrinterChangePostion;
    FPageNumber: Integer;
...
{$ENDIF}
    ...
    property OnPrinterChangePageHeader: TPrinterChangePageHeader read FOnPrinterChangePageHeader write FOnPrinterChangePageHeader;
    property OnPrinterChangeHeadFont: TPrinterChangeHeadFont read FOnPrinterChangeHeadFont write FOnPrinterChangeHeadFont;
    property OnPrinterChangePostion: TPrinterChangePostion read FOnPrinterChangePostion write FOnPrinterChangePostion;
...
function PrinterPreview: TPrinterPreview; overload;
function PrinterPreview(Title: string; DBGridEh: TDBGridEh): TPrinterPreview; overload; // 2010-04-06 liusen Ôö¼Ó¸Äº¯Êý
//function PrinterPreview(PrintDBGridEh: TPrintDBGridEh): TPrinterPreview; overload;
...
procedure TPreviewBox.PrinterChangePageHeader(Value: string);
begin
  //if Assigned(OnPrinterChangePageHeader) then
  //    OnPrinterChangePageHeader(value)
end;
...
function PrinterPreview(Title: string; DBGridEh: TDBGridEh): TPrinterPreview;
begin
  if FPrinterPreview = nil then
  begin
    PreviewFormEh := TPreviewFormEh.Create(Application);
  ...
  PreviewFormEh.EditTitle.Text := Trim(Title);
  //if PreviewFormEh.DBGridEh = nil then
  //  New(PreviewFormEh.DBGridEh);
  PreviewFormEh.DBGridEh := DBGridEh;
  Result := FPrinterPreview;
end;

{function PrinterPreview(PrintDBGridEh: TPrintDBGridEh): TPrinterPreview;
begin
  if FPrinterPreview = nil then
  begin
    PreviewFormEh := TPreviewFormEh.Create(Application);
...
procedure TPrinterPreview.SetPageCaption(const ACaption:String);
begin
  PreviewFormEh.Caption := ACaption; //add by sha ÉèÖÃ½çÃæµÄ±êÌâ¡¡2006-1-20
end;
