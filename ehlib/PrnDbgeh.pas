...
//    property ColumnSpace:Currency read FColumnSpace write SetColumnSpace;
  ...
  protected
    ...
    procedure PrinterChangePageHeader(Value: String);
    procedure PrinterChangeHeadFont(Font: TFont);
    procedure PrinterChangePostion(left: Integer=0; right: Integer=0; top: Integer=0; bottom: Integer=0);
  public
    // liusen
    FKeyValueList: TStringList; // ½«keyvalueµÄÖµÌí¼Óµ½¸ÃÈÝÆ÷ÖÐ
    HaveKeyValue: Boolean;
    OldKeyValue, NewKeyValue: string;
  ...
  published
    ...
    property PageCaption:String read FPageCaption write SetPageCaption;
  end;
...
begin
  if Assigned(OnPrinterSetupDialog) then
    PrinterPreview(PageHeader.CenterText.text, DBGridEh).OnPrinterSetupDialog := OnPrinterSetupDialog
  else
    PrinterPreview(PageHeader.CenterText.text, DBGridEh).OnPrinterSetupDialog := PrinterSetupDialogPreview;
  PrinterPreview(PageHeader.CenterText.text, DBGridEh).OnPrinterChangePageHeader := PrinterChangePageHeader;   // liusen
  PrinterPreview(PageHeader.CenterText.text, DBGridEh).OnPrinterChangeHeadFont := PrinterChangeHeadFont;       // liusen
  PrinterPreview(PageHeader.CenterText.text, DBGridEh).OnPrinterChangePostion := PrinterChangePostion;         // liusen
  PrinterPreview(PageHeader.CenterText.text, DBGridEh).PrinterSetupOwner := Self;
  PrinterPreview(PageHeader.CenterText.text, DBGridEh).SetPageCaption(FPageCaption);//add by sha ÉèÖÃ½çÃæµÄ±êÌâ¡¡2006-1-20
...
//        (DefaultRowHeight mod 2 = 1) and (TopRow mod 2 = 1));
  ...
  begin
    ...
    begin // On width of page
      ...
      if GridWidth > (RealRight - RealLeft) then
      begin
        DataPrintRec.Right := MulDiv(DataPrintRec.Right, GridWidth, RealRight - RealLeft); // Îñòàâèòü ïîëå ñòàðîãî ðàçìåðà
...
//     fPrnPhysOffSetY := GetDeviceCaps(VPrinter_Canvas.Handle,PHYSICALOFFSETY);
    ...
    DataPrintRec.Left := Round(fLogPelsX2 * Page.LeftMargin / Diver) - fPrnPhysOffSetX + FLeft;                     // 2010-04-06 liusen Ôö¼ÓÁË+ FLeft
    DataPrintRec.Top := Round(fLogPelsY2 * Page.TopMargin / Diver) - fPrnPhysOffSetY + FTop;                        // 2010-04-06 liusen Ôö¼ÓÁË+ FTop
    DataPrintRec.Right := fPrnHorsRes - Round(fLogPelsX2 * Page.RightMargin / Diver) + fPrnPhysOffSetX + FRight;    // 2010-04-06 liusen Ôö¼ÓÁË+ FRight
    DataPrintRec.Bottom := fPrnVertRes - Round(fLogPelsY2 * Page.BottomMargin / Diver) + fPrnPhysOffSetY + FBottom; // 2010-04-06 liusen Ôö¼ÓÁË+ FBottom
    ...
    PenW := Trunc((fLogPelsX2 + fLogPelsY2) / 200); // PenWidth = 0.01 Inche
     ...
     {TopFooterPos := DataPrintRec.Bottom - Round(fLogPelsX2 / 4);  // 1/4 äþéìà
...
{$ENDIF}
  ...
  function checkKeyValues(DataRow: Integer): Boolean;
  var
    prValue,value: string;
    prActiveRecord, ActiveRecord: Integer;
  begin
    Result := False;
    if HaveKeyValue then
    begin
      if OldKeyValue = NewKeyValue then
        Result := True;
    end
    else
      Result := False;
    {if FKeyValueList.Count <= 0 then
      Result := True;

    prActiveRecord := DataRow - 1;
    ActiveRecord := DataRow;

    if prActiveRecord < 0 then
      Result := True;

    prValue := FKeyValueList.Strings[prActiveRecord];
    Value := FKeyValueList.Strings[ActiveRecord];
    if prValue = Value then
      Result := True;     }
  end;
  ...
  function DrawCell(ACol: Integer; ARect: TRect; var APBmFrom: TPolyBookmark;
    CalcRect: Boolean; DataPageRow: Integer; writeKeyValue: Boolean): Integer;
  const
...
//      FColCellParamsEh.Font := VPrinter_Canvas.Font;
      ...
      ClearSubLine := False;
      if CurColumn.KeyValue then
      begin
        if writeKeyValue then
        begin
          FKeyValueList.Append(FColCellParamsEh.Text);
          HaveKeyValue := True;
          OldKeyValue := NewKeyValue;
          NewKeyValue := FColCellParamsEh.Text;
        end;
        ...
        begin
          AOldText := FColCellParamsEh.Text;
          if (DataPageRow <> 0) and
            CheckColumnDuplicateValues(CurColumn, DataPageRow, FColCellParamsEh.Text) and checkKeyValues(DataPageRow) then
          begin
            FColCellParamsEh.Text := '';
            FColCellParamsEh.ImageIndex := -1;
            ...
            // liusen
            ClearSubLine := True;
            //VPrinter_Canvas.Pen.Color := clWhite;

            //color := VPrinter_Canvas.Pen.Color;
            //VPrinter_Canvas.Pen.Color := clRed; //CurColumn.Color;
            //VPrinter_Canvas.Polyline([Point(ARect1.Left, ARect1.Top-1),
            //                   Point(ARect1.Right,ARect1.Top-1)]);
            //VPrinter_Canvas.Pen.Color := color;
...
//        SetPen;
  ...
  function CalcPrintRows(APBmFrom: TPolyBookmark; PrnTop, PrnBottom: Integer; writeKeyVaule: Boolean): TPolyBookmark;
...
//    PrnRowHeights.Clear;
    ...
    begin
      ...
      begin
        ...
        rh := DrawCell(i, r, APBmFrom, True, -1, writeKeyVaule);
        if ch < rh then ch := rh;
      end;
...
//      PrnRowHeights.Add(Pointer(ch));
  ...
  // Êä³öÁÐ±íÖÐµÄÄÚÈÝ¡¢»­³öÁÐ±íÏß
  function DrawRowsRange(var APBmFrom, APBMTo: TPolyBookmark; writeKeyValue: Boolean): Integer;
...
//      PrintPageColontitle(FooterRect,False,False);
      ...
      begin
        ...
        begin
          ...
          DrawCell(i, ARect1, CurPBm, False, j, writeKeyValue);

          CurColumn := PrnColumns[i];
          if CurColumn.HideDuplicates then
          begin
            AOldText := FColCellParamsEh.Text;
            ...
            begin
              ...
              begin         }
                AddPolyline([ARect1.Right, ARect1.Top, ARect1.Left, ARect1.top, ARect1.Left, ARect1.Bottom]);

              //end;
            end;
            //if Length(PrnRowHeights)-1 = j then
            //begin
            //  AddPolyline([ARect1.Right, ARect1.top, ARect1.Left, ARect1.top]);
            //end;
            AddPolyline([ARect1.Right, ARect1.Bottom, ARect1.Right, ARect1.Top]);
            AddPolyline([ARect1.Left, ARect1.Top, ARect1.Left, ARect1.Bottom]);
          end
          else
          begin
            if ((i = FromCol) and not DBGridEh.UseRightToLeftAlignment) or
          ...
          if not CurPBm.InDS then    // Êý¾ÝÁÐ±íÓÐÍ³¼ÆÊ±£¬ÔÚÍ³¼Æ
          begin
            AddPolyline([ARect1.Right, ARect1.Top, ARect1.Left, ARect1.Top]);
          end;  
        ...
        if Length(PrnRowHeights) - 1 = j then
        begin
          AddPolyline([Left, Bottom, Right, Bottom]);      // Ò»Ò³½áÊøºó£¬ÔÚ±¾Ò³ÁÐ±í×îÏÂÃæ»­Ò»ÌõÏß£¬·âµ×£¬²»È»¿ÉÄÜ³öÏÖÈ±¿Ú
          Break;
        end;
...
{$ENDIF}
  ...
  begin
    ...
    ANode.HeightPrn := maxh; //ñîõðàíÿåì âûñîòó ChildTree â Õîñòå
    Result := maxh;
  end;
...
//        EndDoc;
  ...
  begin
    ...
    while True do begin
      PBm2 := CalcPrintRows(PBm1, DataPrintRec.Top + PrintOffset, DataPrintRec.Bottom, False);
      PrintOffset := DrawRowsRange(PBm1, PBm2, True) + PenW;
      if (PBm2.EOF = True)
        then Break
      else PrintOffset := 0;
      PBm1 := PBm2;
...
//    PolyLengths := TList.Create;
    ...
    // ±¨±í±êÌâµÄÎ»ÖÃ 2010-04-06 liusenÔö¼ÓÁË-FLeft\-FTop\-FRight\-FTop
    TitleRect := Rect(DataPrintRec.Left-FLeft, DataPrintRec.Top-FTop, DataPrintRec.Right-FRight, DataPrintRec.Top-FTop);
    Inc(TitleRect.Bottom, PrintPageColontitle(TitleRect, True, True));
    Inc(DataPrintRec.Top, TitleRect.Bottom - DataPrintRec.Top);

    // ±¨±íÒ³½ÅµÄÎ»ÖÃ 2010-04-06 liusenÔö¼ÓÁË-FLeft\-FBottom\-FRight\-FBottom
    FooterRect := Rect(DataPrintRec.Left-FLeft, DataPrintRec.Bottom-FBottom, DataPrintRec.Right-FRight, DataPrintRec.Bottom-FBottom);
   ...
   //FromRow := 0; PageNo := 1;
    ...
    Inc(DataPrintRec.Top, PrnTitlEh + PrnTitleTextH+FTop); // 2010-04-07  liusen Ôö¼ÓÁË+FTop Ê¹±¨±íÁÐ±í¿ÉÒÔÏòÏÂÒÆ¶¯£¬¶ø±êÌâÎ»ÖÃ²»±ä
    ...
    begin
      ...
      try
        ...
        PrintOutInfo;
        //----Add by sha 20090320¡£½â¾ö´íÎó£ºÈ¡'&[Pages]'(¹²¼¸Ò³) »áÔì³ÉºÏ¼ÆÐÐÊý¾ÝÎª2±¶
        for i := 0 to DBGridEh.FooterRowCount - 1 do
          for j := 0 to PrnColumns.Count - 1 do
...
procedure TPrintDBGridEh.PrinterChangePageHeader(Value: String);
begin
  Self.PageHeader.CenterText.Clear;
  //Self.PageHeader.CenterText.add(Value);
  Self.PageHeader.CenterText.Text := Value;
  Preview;
end;

procedure TPrintDBGridEh.PrinterChangeHeadFont(Font: TFont);
begin
  Self.PageHeader.Font := Font;
  Preview;
end;

procedure TPrintDBGridEh.PrinterChangePostion(left: Integer=0; right: Integer=0; top: Integer=0; bottom: Integer=0);
begin
  Self.FLeft := left;
  Self.FRight := right;
  Self.FTop := top;
...
procedure TPrintDBGridEh.SetPageCaption(const Value: String);
begin
  FPageCaption := Value;
end;
