...
type
  TQueryPictureFrm = class(TBaseFrm)
    Label1: TLabel;
...
var
  QueryPictureFrm: TQueryPictureFrm;

implementation
uses uFrmSelectDevs,Carunit,uGloabVar,ConstDefineUnit;
{$R *.dfm}

procedure TQueryPictureFrm.CheckBox1Click(Sender: TObject);
begin
...
procedure TQueryPictureFrm.BitBtn1Click(Sender: TObject);
var
  i,j:integer;
  D:TDevice;
...
begin
  try
    ...
    begin
      ...
      if CheckBox1.Checked then
      begin
        DataServer.SendQueryPicture_V3(D, ComboBoxCamera.ItemIndex, ComboBoxReason.ItemIndex,
                                       FormatDatetime('yyyy-mm-dd hh:nn:ss',beginDateTime),
                                       FormatDatetime('yyyy-mm-dd hh:nn:ss',endDateTime), true);
      end
      else
        DataServer.SendQueryPicture_V3(D, ComboBoxCamera.ItemIndex,  ComboBoxReason.ItemIndex, '', '', False);
      ProgressBar1.StepIt;
      ProgressBar1.StepIt;
      //Sleep(50);
...
procedure TQueryPictureFrm.FormShow(Sender: TObject);
begin
...
procedure TQueryPictureFrm.ComboBoxCameraKeyPress(Sender: TObject;
  var Key: Char);
begin
  Key := #0;
end;

procedure TQueryPictureFrm.ComboBoxReasonKeyPress(Sender: TObject;
  var Key: Char);
begin
  Key := #0;
end;
