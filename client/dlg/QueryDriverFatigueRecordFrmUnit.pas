unit QueryDriverFatigueRecordFrmUnit;
...
type
  TQueryDriverFatigueRecordFrm = class(TForm)
...
var
  QueryDriverFatigueRecordFrm: TQueryDriverFatigueRecordFrm;
  group_id: integer = -1;
...
procedure TQueryDriverFatigueRecordFrm.ExpandTreeView;
var
  i: integer;
begin
...
procedure TQueryDriverFatigueRecordFrm.InitEdit;
var
  i: Integer;
begin
...
procedure TQueryDriverFatigueRecordFrm.ListGroup;
...
procedure TQueryDriverFatigueRecordFrm.SpeedButtonTjDayClick(Sender: TObject);
var
...
begin
  ...
  try
    ...
    ClientDataSet1.Data := BS.QueryDriverFatigueRecord(fromDatetime, toDatetime,devId,group_id);
...
procedure TQueryDriverFatigueRecordFrm.FormCreate(Sender: TObject);
begin
  InitEdit;
  ListGroup;
  PageControl1.ActivePageIndex := 0;
...
procedure TQueryDriverFatigueRecordFrm.TreeView1Click(Sender: TObject);
...
procedure TQueryDriverFatigueRecordFrm.AddDev(id: integer; ANode: TTreeNode);
var
  i: integer;
  dev: TDevice;
begin
...
procedure TQueryDriverFatigueRecordFrm.SpeedButtonPrintDayClick(Sender: TObject);
var
...
begin
  try
    ...
    fromDatetime :=FormatDatetime('yyyy-mm-dd ',fromDate.Date)+FormatDatetime('hh:nn:ss',fromTime.Time);
    toDatetime:= FormatDatetime('yyyy-mm-dd ',toDate.Date)+FormatDatetime('hh:nn:ss',toTime.Time);
    str := s+ ' 疲劳驾驶报表'+#13#10#13#10+'时段：'+fromDatetime+'至'+toDatetime;
...
procedure TQueryDriverFatigueRecordFrm.BitBtnCloseClick(Sender: TObject);
begin
  close;
end;

procedure TQueryDriverFatigueRecordFrm.BitButtonExcelDayClick(Sender: TObject);
var
  i, j: Integer;
  s: string;
  Str: string;
...
begin
  ...
  try
    ...
    //报表头
      ...
      fromDatetime :=FormatDatetime('yyyy-mm-dd ',fromDate.Date)+FormatDatetime('hh:nn:ss',fromTime.Time);
      toDatetime:= FormatDatetime('yyyy-mm-dd ',toDate.Date)+FormatDatetime('hh:nn:ss',toTime.Time);
      str :=fromDatetime+'至'+toDatetime+ s+ ' 疲劳驾驶报表';
    ...
    //取数
      ...
      MyWorkbook.worksheets[1].Cells[i, 6].value := DBGridEh1.Columns[5].Footer.SumValue; 
    //设置单元格格式
      ...
      MyWorkbook.worksheets[1].range['F3:F'+IntToStr(ClientDataSet1.RecordCount+3)].NumberFormatLocal := '0.00';
...
procedure TQueryDriverFatigueRecordFrm.FormClose(Sender: TObject;
