...
type
  TSelect_Devs = class(TForm)
    ...
    DestDispLabel: TcxTreeListColumn;
...
var
  Select_Devs: TSelect_Devs;

implementation
 uses uGloabVar,ConfineAreaUnit,DateUtils,umainf ;
{$R *.dfm}

procedure TSelect_Devs.FormCreate(Sender: TObject);
begin
  ListAllDev.Execute;
...
procedure TSelect_Devs.ListAllDevExecute(Sender: TObject);
    //子组的组ID号， 节点，当前还余下数
...
procedure TSelect_Devs.TreeView1Click(Sender: TObject);
var
  tmpGrpId : integer;
  i,j:integer;
  dev: Tdevice;
...
procedure TSelect_Devs.Add2DevList(Alist:TcxTreeList; const ADev: Tdevice);
var
...
procedure TSelect_Devs.Add2DevListToFrom(Alist:TcxTreeList; const ADev: Tdevice);
begin
  try
...
procedure TSelect_Devs.FillData(ADev: TDevice; ANode: TcxTreeListNode);
begin
  with ADev,ANode do
  begin
    Data := ADev;
...
procedure TSelect_Devs.FromListClick(Sender: TObject);
begin
...
procedure TSelect_Devs.FromListFocusedNodeChanged(Sender: TObject;
...
procedure TSelect_Devs.DestListClick(Sender: TObject);
begin
...
procedure TSelect_Devs.DestListFocusedNodeChanged(Sender: TObject;
  APrevFocusedNode, AFocusedNode: TcxTreeListNode);
begin
//  if DestList.FocusedNode = nil  then exit;
//  FDestSelectDev :=TDevice(DestList.FocusedNode.Data);
...
procedure TSelect_Devs.AddSelectExecute(Sender: TObject);
begin
...
procedure TSelect_Devs.AddAllExecute(Sender: TObject);
begin
...
procedure TSelect_Devs.InsertDev2DestList(Adev: TDevice);
var
...
procedure TSelect_Devs.BitBtn_okClick(Sender: TObject);
begin
  ...
  begin
    messagebox(handle,pchar('请重新输入'+LabelTjTimeSet.Caption),'提示',mb_ok + mb_iconinformation);
    exit;
  end
  else
    ModalResult := mrOk;
...
procedure TSelect_Devs.DeleteSelectExecute(Sender: TObject);
begin
  ...
  FSelectDevGrpID:=FSelectDevGrpID+1;
  TreeView1Click(TreeView1);
...
procedure TSelect_Devs.DeleteAllExecute(Sender: TObject);
var
  dev:TDevice;
  i: integer;
begin
...
function  TSelect_Devs.isInDestList(Dev:TDevice):Boolean;
var
 i:integer;
begin
  Result:=false;
...
procedure TSelect_Devs.FormClose(Sender: TObject;
