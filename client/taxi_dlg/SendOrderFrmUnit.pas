...
type
  ...
  public
    ...
    procedure ShowAllinfo(ID,OderTime,CustomerTel,OrderContent,OrderDetailContent,SucceedDevID,State,OperationUserName:string;orderDevID:array of string );
...
begin
      ...
      RadioGroupOrderType.ItemIndex:=1;
      ...
      EditWhoSend.Text:=Order.OperationUserName;
      ListView1.Clear;
...
procedure TSendOrderFrm.ShowAllinfo(ID,OderTime,CustomerTel,OrderContent,OrderDetailContent,SucceedDevID,State,OperationUserName:string;orderDevID:array of string);
var
 i:integer;
 s:String;
begin
      ...
      EditOrderID.Text:=ID;
      RadioGroupOrderType.ItemIndex:=1;
      DateTimePickerDate.Date:=strtodateTime(OderTime);
      DateTimePickertime.Time:=strtodateTime(OderTime);
      EditCustomTel.Text:=CustomerTel;
      Memo1.Text:=OrderContent;
      Memo2.Text:=OrderDetailContent;
      EditOKCarNO.Text:=FindDevice(strtoint(SucceedDevID));
      EditState.Text := State;
      {if  state='5' then
      begin
        EditState.Text:='抢单成功';
      end else
      begin
        EditState.Text:='中心主动删除';
      end;    }
      EditWhoSend.Text:=OperationUserName;
      ListView1.Clear;
      for i:=0 to   length(orderDevID)-1 do
      begin
        //s:=ACarNOManage.GetCarNO(orderDevID[i]);
        with ListView1.Items.Add do
        begin
          Caption:=orderDevID[i]; //s;
        end;
      end;
end;
